...
def _smooth_spectra(spectra, kernel, decim=1):
    ...
    # smooth the spectra
    spectra = fftconvolve(spectra, kernel, mode='same', axes=(-2, -1))
...
def conn_coherence_wav(
    ...
    mode='morlet', n_cycles=7., mt_bandwidth=None, decim=1, decim_at='tfd', kw_cwt={},
    ...
    data, cfg = conn_io(
        ...
        pairs=pairs, sort=True, block_size=block_size, sfreq=sfreq,
        name=f'Coherence (mode={mode})', verbose=verbose,
    )
    ...
    # decim_at should be either 'tfd' or 'coh'
    assert decim_at in ['tfd', 'coh']
    ...
    if need_mean:
        ...
        if isinstance(decim, int): 
            win_times = win_times[::decim]  # noqa
            # Set parameters in case decim is done at tf decomp. 
            # level or at coherence computation level
            if decim_at == 'tfd':
                decim_tfd=decim
    ...
    # _______________________________ COHERENCE _______________________________
    # compute coherence on blocks of trials
    coh = np.zeros((n_trials, n_pairs, len(f_vec), len(win_times)))
    for tr in indices:
        # --------------------------- TIME-FREQUENCY --------------------------
        ...
        parallel, p_fun = parallel_func(
            ...
            total=n_pairs, mesg='Estimating pairwise coherency')
    ...
    coh = xr.DataArray(coh, dims=('trials', 'roi', 'freqs', 'times'),
                       name='coh', coords=(trials, roi_p, f_vec, win_times),
                       attrs={**attrs, **cfg})
    return coh
...
def conn_coherence_psd(
    ...
    # inputs conversion
    data, cfg = conn_io(
        data, times=times, roi=roi, agg_ch=False, win_sample=win_sample,
        pairs=pairs, block_size=block_size, sfreq=sfreq, sort=True,
        name='Coherence (PSD)', verbose=verbose,
    )

    # extract variables
    x, trials, attrs = data.data, data['y'].data, cfg['attrs']
    n_trials, sfreq = len(trials), cfg['sfreq']
...
if __name__ == '__main__':
    ...
    sl_1, sl_2 = slice(0, 500), slice(500, 1000)

    """
    - 10hz coherence between samples [0, 500]
    - 40hz coherence between samples [500, 1000]
    """
    x = np.random.rand(n_trials, n_roi, n_times)
    x[0:5, ..., sl_1] += np.sin(2 * np.pi * 10 * times[sl_1]).reshape(1, 1, -1)
    x[5::, ..., sl_2] += np.sin(2 * np.pi * 40 * times[sl_2]).reshape(1, 1, -1)

    x = xr.DataArray(x, dims=('trials', 'roi', 'times'),
                     coords=(trials, roi, times))
    freqs = np.linspace(2, 60, 100)
    n_cycles = freqs / 2.
    win_sample = np.array([[0, 500], [250, 750] ,[500, 999]])

    foi = np.array([[2, 4], [5, 7], [8, 13], [13, 30], [30, 60], [60, 100]])
    coh = conn_coherence_wav(
        x, freqs, sfreq=sfreq, roi='roi', times='times', sm_times=300,
        sm_freqs=1, mode='multitaper', n_cycles=n_cycles,
        win_sample=win_sample, decim=10, foi=None, block_size=1, n_jobs=1)

    # win_sample = define_windows(times, slwin_step=0.01, slwin_len=4.)[0]
    # coh = conn_coherence_psd(
    #     x, sfreq=sfreq, roi='roi', times='times', block_size=1, foi=None,
    #     win_sample=win_sample,
    #     kw_coh=dict(nfft=1000, nperseg=256, noverlap=200))
    print(coh)
