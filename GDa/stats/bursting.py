...
@nb.jit(nopython=True)
def masked_find_activation_sequences(spike_train, mask, dt=None,
                                     find_zeros=False, drop_edges=False,
                                     pad=False):
    """
    Similar to "find_activation_sequences" but a mask is applied to the
    spike_train while computing the size of the activation sequences.
    ...
    pad: int, float | False
        ...
        If the spike-train contains n samples the number of activation
        sequences is constrained between 0 (no activation of the edge during
        the period) and ceil(n/2) (which would correspond to an activation
        time-series with 0s and 1s intercalated). If pad is True the series
        returned will have size ceil(n/2) by being padded with zeros
        to the right.
    ...
    act_lengths = find_activation_sequences(
        spike_train[mask], dt=dt, find_zeros=find_zeros)
    ...
    if drop_edges:
        ...
        if i >= 1:
            if spike_train[i-1] == edge_value and spike_train[i] == edge_value and len(act_lengths) > 0:
        ...
        if j < len(mask)-1:
            if spike_train[j] == edge_value and spike_train[j+1] == edge_value and len(act_lengths) > 0:
...
def tensor_find_activation_sequences(spike_train, mask, dt=None,
                                     find_zeros=False, drop_edges=False,
                                     n_jobs=1):
    """
...
def tensor_burstness_stats(spike_train, mask, drop_edges=False,
                           find_zeros=False, samples=None, dt=None,
                           n_jobs=1):
    """
    Given a data tensor (links,trial,time) composed of spike trains the sequence
    of activations of it will be determined
    (see tensor_find_activations_squences) and
    ...
    mask: array_like
        Binary mask applied to the spike-train with size [trials, time].
        For more than one mask a dicitionary should be provided where for
        each key an array with size [trials, time] is provided.
    drop_edges: bool | False
        If True will remove the size of the last burst size in case
        the spike trains ends at one.
    find_zeros: bool | False
    ...
    -------
    bs_stats: array_like
        array containing mu, mu_tot, and CV computed from the activation
        sequences in the spike train with shape [links,4] or [links,stages,4].
    """
    if dt is None:
        dt = 1

    # Computing activation lengths
    out = tensor_find_activation_sequences(spike_train, mask, dt=dt,
                                           find_zeros=find_zeros,
                                           drop_edges=drop_edges,
                                           n_jobs=n_jobs)
    ...
    if isinstance(out, np.ndarray):
        ...
        for idx, key in enumerate(out.keys()):
            ...
            bs_stats[:, idx, 2] = np.nansum(
                out[key], axis=-1)/(samples[idx]*dt)
            bs_stats[:, idx, 3] = (
                bs_stats[:, idx, 1]-bs_stats[:, idx, 0])/(bs_stats[:, idx, 1]+bs_stats[:, idx, 0])
    return bs_stats
