...
parser.add_argument("METRIC", help="which FC metric to use", type=str)
parser.add_argument("SURR", help="wheter to compute original or surrogate MC", type=int)
parser.add_argument("THR", help="wheter to threshold or not the coherence", type=int)
...
if surr == 2:
    ...
    FC = (
        FC.transpose("roi", "freqs", "trials", "times")
        .stack(obs=("trials", "times"))
        .data
    )
else:
    # Stack trials
    ...
    for ti, tf in tqdm(stages):
        FC += [
            net.super_tensor.sel(times=slice(ti, tf))
            .stack(obs=("trials", "times"))
            .data
        ]
...
MC = xr.DataArray(
    ...
    coords={
        "sources": net.super_tensor.roi.values,
...
# Saving data
_PATH = os.path.expanduser(os.path.join(_ROOT, f"Results/{monkey}/meta_conn"))

# Save MC
if bool(surr):
    # Saving MC
    MC.to_netcdf(os.path.join(_PATH, f"MC_{metric}_{session}_at_{at}_surr_{surr}.nc"))
    ...
    # Saving MC
    MC.to_netcdf(os.path.join(_PATH, f"MC_{metric}_{session}_at_{at}_thr_{thr}.nc"))
    # Compute entanglement
    save_path_ent = os.path.join(
