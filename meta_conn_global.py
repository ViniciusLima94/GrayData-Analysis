...
def convert_to_stream(adj, sources, targets, dtype=np.float32):
    ...
    def _convert(adj):
        ...
        tensor = np.zeros([n_pairs, n_bands], dtype=dtype)
...
for session in tqdm(sessions):
    ...
    for f in range(temp.shape[-1]):
        np.fill_diagonal(temp[..., f].values, 1)
    MC += [temp]
...
temp = xr.DataArray(
    np.zeros((len(unique_rois), len(unique_rois), 10)),
    dims=("sources", "targets", "freqs"),
