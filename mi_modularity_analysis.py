...
if avg == 0:
    ...
    mi_sig.squeeze().plot.imshow(x="times",
                                 y="freqs",
                                 cmap="turbo",
                                 vmax=0.01,
                                 interpolation="gaussian")
    plt.title('MI(modularity, stimulus)')
    plt.savefig(f"figures/mi_{metric}_avg_{avg}.png", bbox_inches='tight')
else:
    plt.figure(figsize=(12,4))
    mi_sig.squeeze().plot(x="times", hue="freqs")
    plt.title('MI(avg. modularity, stimulus)')
    plt.savefig(f"figures/mi_{metric}_avg_{avg}.png", bbox_inches='tight')
