...
##################################################################################
for i in range(len(net.bands)):
    Q['coreness'][str(i)] = compute_nodes_coreness(
        net.A[:, :, i, :], is_weighted=False)
...
##################################################################################
for i in range(len(net.bands)):
    Q['clustering'][str(i)] = compute_nodes_clustering(
        net.A[:, :, i, :], is_weighted=False)
...
##################################################################################
for i in range(len(net.bands)):
    Q['modularity'][str(i)] = compute_network_modularity(
        net.A[:, :, i, :], is_weighted=True)
...
# SAVING DICTIONARIES
##################################################################################
np.save('network_statistics/tt_'+str(trial_type)+'_br_' +
        str(behavioral_response)+'.npy', {'original': Q, 'null': Qr})
