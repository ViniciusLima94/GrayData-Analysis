...
fbands    = ['6-8Hz', '10-14Hz', '16-30Hz', '30-60Hz']  # Frequency band labels
Nbands    = len(fbands)
# Reading info about data
info  = scio.loadmat(session['dir_out']+'readinfo.mat')
# Channel labels
labels= info['channels_labels']
# Pairs index
pairs = info['pairs']
# Number of trials
nT  = info['nT'][0,0]
...
nt  = len(info['time'][0])
# Cue number 
cue = info['trialinfo'][:,1] 
# Pair index
p_idx = np.arange(0, nP)
# Time array
time = info['time'][0]
...
# Cue duration (averaged over trials)
dcue = np.mean( info['dcue'][0] / 1000.0 )
# Distance sample-math (averaged over trials)
dsm  = np.mean( info['dsm'][0]  / 1000.0 )
# Stimulus
stimulus = info['stim'][0]
#
f    = 0.3
# For plotting the brainsketch
xy                = scio.loadmat('Brain Areas/lucy_brainsketch_xy.mat')['xy'] # Channels coordinates
ethyl_brainsketch = simg.imread('Brain Areas/ethyl_brainsketch.jpg')          # Brainsketch 

stages = ['before_cue', 'cue', 'early_delay', 'late_delay', 'match']